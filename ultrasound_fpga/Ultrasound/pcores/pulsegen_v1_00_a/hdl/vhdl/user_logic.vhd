...
TX_Addr <= slv_reg0(3 downto 0);

PULSE_GEN_PROC : process( Bus2IP_Clk ) is
  begin
		...
		case state is
			when S_RST =>
				if slv_reg0(16) = '1' then
					--Microblaze has written to slv_reg0, device enabled
					cycle_counter <= conv_integer(slv_reg0(15 downto 4));
					state <= S_START;
				end if;
			when S_START =>
